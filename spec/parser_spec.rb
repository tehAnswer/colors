RSpec.describe Colors::Parser do
  ...
  context 'valid script' do
    let(:file_path) { File.expand_path('../shared/valid_script.txt') }
    let(:commands) do
      [
        Colors::Commands::Init.new(rows: 10, columns: 10),
        Colors::Commands::Clear.new,
        Colors::Commands::Pixel.new(row: 1, column: 3, color: :black),
        Colors::Commands::VerticalLine.new(column: 1, start_row: 1, end_row: 3, color: :white),
  ...
  context 'invalid script (unknown command)' do
    let(:file_path) { File.expand_path('../shared/unknown_command.txt') }
  ...
  context 'conflicting script (extra spaces)' do
    let(:file_path) { File.expand_path('../shared/extraspaced_script.txt') }
    let(:commands) do
      [
        Colors::Commands::Init.new(rows: 10, cilumns: 10),
  ...
  context 'conflicting script (empty)' do
    let(:file_path) { File.expand_path('../shared/empty_script.txt') }

    it { is_expected.to be_empty }
  end

  context 'conflicting script (lower case)' do
    let(:file_path) { File.expand_path('../shared/lower_case_script.txt') }
  ...
  context 'conflicting script (syntax vs semantics)' do
    let(:file_path) { File.expand_path('../shared/invalid_semantics_init.txt') }
  ...
  Dir['../shared/wrong_arguments_*', __FILE__].each do |file|
    context 'invalid script (wrong call)' do
      let(:file_path) { file }

      it "#{file.split('/').last.split('.').first} throws error" do
        expect { subject }.to raise_error(Colors::ParseError)
      end
    end
  end
